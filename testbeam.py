...
parser.add_argument('--calib', dest='calib', action = "store_true", default=False, help='Execute the calibration for track matching')
...
sample_type = ""
if args.npix or args.tot or args.eff or args.calib:
    sample_type = 'track-data'
else:
...
for f in skim_samples:
    ...
    if args.calib:
        cmd = "root -l -q 'root/calib.C("+'"'+str(f)+'"'+")'"
        os.system(cmd)
    if args.npix:
