...
double npix(TString filename, TString mode){
  ...
  TTree* hit_tree = (TTree*) dut_dir->Get("Hits");
  ...
  hit_tree->SetBranchAddress("NHits",&NHits);
  hit_tree->SetBranchAddress("PixX",&HitPixX);
  hit_tree->SetBranchAddress("PixY",&HitPixY);
  hit_tree->SetBranchAddress("HitInCluster",&HitInCluster);
  ...
  for (int i = 0; i< ENTRIES; i++)
      ...
      for (int ic = 0; ic < NClusters; ic++)
	{
	  ...
	  for (int ip = 0; ip < NHits; ip++)
	    {
	      //std::cout<<"pixel "<<ip<<" x "<<HitPixX[ip]<<" y "<<HitPixY[ip]<<std::endl;
	      Bool_t isMatchingX = fabs(HitPixX[ip]-Col[ic])<NMAXRANGE;
	      Bool_t isMatchingY = fabs(HitPixY[ip]-Row[ic])<NMAXRANGE;
	      if (isMatchingX && isMatchingY) nPix++;
	    }
